...
interface DebitTabProps {
  account: LinkedAccount | null;
}

const DebitTab: React.FC<DebitTabProps> = ({ account }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const res = await fetch('/api/transfer/debit', {
        method: 'POST',
  ...
  return (
    <div>
      <p style={{ color: '#374151', fontSize: '14px', marginBottom: '16px' }}>
        Pull funds from <strong>{account.institution_name}</strong> (••••{account.account_mask}).
      </p>
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '12px', maxWidth: '320px' }}>
        ...
        >
          {submitting ? 'Processing…' : 'Initiate Debit'}
        </button>
      </form>
